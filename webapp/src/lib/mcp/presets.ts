 ...
 * MCP servers chosen for WhiteHat's pentest workflow.
 *
...
export const MCP_PRESETS: McpPreset[] = [
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Research vulnerable libraries, understand exploit code in public PoC repos, audit dependencies before targeting them.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Find exploit PoCs, search for credentials in public repos, identify vulnerable code patterns, supply-chain analysis.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Find security research papers (e.g., recent CVE writeups, exploitation techniques), datasets for fuzzing, ML-based vuln-detection tools.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Look up current API surface of libraries you\'re probing (e.g., to understand authentication flows or find deprecated/removed protections).',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Independent OSINT search engine - useful when Tavily/Google rate limits hit during recon, or for cross-checking results from a different index.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Carry findings across sessions: track confirmed credentials, persistent shells, mapped subdomain → CVE relationships beyond a single chat.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Force the LLM to plan exploit chains explicitly: enumerate steps, revise hypothesis, document decision points. Audit-trail-friendly.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Stash exfiltrated files, build wordlists/payloads incrementally, save reverse-shell artifacts between iterations. Sandboxed to /app/logs/sandbox.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Map external attack surface, find exposed services, banner-grab without touching the target. Augments execute_naabu by adding pre-existing scan data + CVE matches.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Triage suspicious artifacts found during scans, check IP reputation before exfil callbacks, identify known-malicious domains hosted on target subdomains.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Single-call DNS recon (lookups, reverse, SPF chain, DMARC, wildcards, SRV), CT log search (crt.sh), passive ASN/BGP analysis. Fills gaps the kali-sandbox tools don\'t cover natively.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'IP reputation enrichment - distinguish opportunistic scanners from targeted attackers, validate exfil destinations, check WAF blocklists. Multiple feeds cross-corroborate.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Backup for built-in web_search if Tavily key isn\'t set globally; also exposes extract/crawl tools the built-in lacks. Useful when scoping a target - fetch their entire site.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Find writeups by description ("recent CVE-2024 RCEs in Java deserializers"), discover similar PoCs to a known exploit, semantic search for vendor-specific advisories.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Stealth-mode OSINT search that doesn\'t correlate with your project\'s Tavily/Brave keys. Useful when you want zero attribution to your infra during attribution-sensitive recon.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Once SQLi grants DB access, point this at the captured DSN to enumerate schemas/tables/users/permissions structurally - much cleaner than crafting SQL through the injection point.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Notify your team Slack when the agent confirms a vuln, completes a phase, or needs human approval. Also useful as a low-noise output channel during long unattended scans.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Background on a target organization, vendor history, biographical info on email enumeration targets. Neutral source independent of your search-API rate limits.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Drives ZAP\'s active scanner from the agent - automated XSS/SQLi/SSRF/path-traversal hunts on web targets, with payload variation that nuclei templates don\'t cover.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Independent corroboration of Shodan data; Censys often catches assets Shodan misses. Strong on TLS/cert intel - find shadow IT subdomains via SAN-list mining.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Phishing simulation prep, social-engineering target enumeration, AD username-pattern discovery (often emails == usernames). Free tier 25 req/month.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Check whether emails captured during recon appear in known breaches → predict valid passwords for credential stuffing. Domain-wide search reveals breach exposure for the entire target.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Active web-app pentest beyond what curl can do - capture mobile-app / SPA traffic, manipulate JWT tokens mid-flight, replay session-fixation payloads. Complements execute_playwright.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Triage exfiltrated SQLite files (mobile-app DBs, Chrome cookies/history, Slack desktop, Signal, password managers). Many high-value artifacts ship as .db files.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Stream findings into a structured engagement workspace as the agent confirms vulns. Pages can become draft sections of the final pentest report. 22 tools.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'When the local Playwright/Puppeteer hits aggressive bot detection (Cloudflare Turnstile, PerimeterX, Akamai), Browserbase provides residential-IP-backed sessions with anti-detection profiles.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Scan target source you\'ve exfiltrated or have access to (open-source dependencies, package.json, requirements.txt, etc.) for known CVEs and license risks.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'When pentesting an e-commerce target that uses Stripe, you can validate the merchant\'s test-mode flow, simulate fraudulent card patterns, and check webhook signature verification with safe test keys.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Convert each confirmed vulnerability into a Linear issue automatically (severity, repro steps, evidence). Sync with the client\'s remediation team during long engagements.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Scan target Docker images / Kubernetes manifests / Terraform after exfil - identifies vulnerable base images, hard-coded secrets, IaC misconfig (open security groups, public S3, etc).',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'When you obtain cloud credentials, runs 300+ checks against the target tenancy: IAM mistakes, public S3, weak RDS encryption, open NSGs, etc. Catches things the basic AWS MCP can\'t.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'Deeper CVE context than the built-in cve_intel tool: EPSS exploitation probability, CISA known-exploited status, ATT&CK technique mappings, OSV cross-reference. Useful for prioritizing exploit targets.',
  ...
  // -------------------------------------------------------------------------
  {
    ...
    whyForWhiteHat: 'When you exfiltrate proprietary binaries (Linux ELF, Windows PE, mobile-app native libs), this lets the LLM disassemble / decompile / search strings + xrefs to find auth bypasses, hard-coded secrets, vulns.',
