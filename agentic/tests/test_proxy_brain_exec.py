...
The live NameError ("whitehat is not defined") slipped through because every other
test imports `whitehat` directly and never exercised the proxy_brain MCP wrapper,
...
def test_proxy_brain_wrapper_pre_imports_whitehat():
    # Regression guard for the live NameError: the wrapper MUST prepend an import
    # so `whitehat.*` works in agent code that does not import it. Asserted from
    ...
    assert "import whitehat" in body, "proxy_brain must pre-import whitehat for the child"
...
def test_txn_parses_summary_line():
    ...
    t = whitehat.Txn("abc123", "abc123  GET 200 pbtarget:5000/api/x?y=1  75b  agent [auth]")
    assert t.id == "abc123"
    assert t.method == "GET"
    assert t.status == 200
...
def test_txn_tolerates_short_or_empty_line():
    t = whitehat.Txn("id", "id")
    assert t.method is None and t.status is None and t.url is None and t.host is None


def test_sdk_fails_closed_without_ctx():
    # Without the injected WHITEHAT_CTX the SDK must refuse (SystemExit from _die),
    # never issue a tenant-less request.
    old = os.environ.pop("WHITEHAT_CTX", None)
    try:
        with pytest.raises(SystemExit):
            whitehat.search(host="x")
    finally:
        if old is not None:
            os.environ["WHITEHAT_CTX"] = old
