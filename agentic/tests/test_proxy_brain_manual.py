...
`whitehat.manual()` before writing complex code. If it documents a function that
...
# The public SDK the manual promises the agent. Keep in sync with whitehat.py.
PUBLIC_API = {
...
def test_every_documented_function_exists():
    ...
    with open(whitehat._MANUAL_PATH, "r", encoding="utf-8") as fh:
        full = fh.read()
    documented = set(re.findall(r"whitehat\.(\w+)\s*\(", full))
    ...
    missing = sorted(d for d in documented
                     if not callable(getattr(whitehat, d, None)))
    assert not missing, f"manual documents non-existent whitehat.* functions: {missing}"


def test_manual_recipes_use_no_private_helpers():
    # Recipes must use the PUBLIC API only — no whitehat._b64u / _run / _post etc.,
    # which are internal and may change without notice.
    with open(whitehat._MANUAL_PATH, "r", encoding="utf-8") as fh:
        full = fh.read()
    privates = sorted(set(re.findall(r"whitehat\.(_\w+)", full)))
    assert not privates, f"manual leaks private SDK helpers into recipes: {privates}"


def test_every_public_function_is_documented():
    core = whitehat.manual()
    undocumented = sorted(fn for fn in PUBLIC_API if f"whitehat.{fn}" not in core)
...
def test_public_api_matches_sdk():
    ...
    missing = sorted(fn for fn in PUBLIC_API if not callable(getattr(whitehat, fn, None)))
    assert not missing, f"PUBLIC_API lists functions not on the SDK: {missing}"


def test_section_retrieval_and_index_consistency():
    core = whitehat.manual()
    secs = [s for s in whitehat._manual_sections() if s]
    ...
    for name in ("recon", "intruder", "sqli", "authz", "jwt", "race",
        ...
        body = whitehat.manual(name)
        assert len(body) > 50 and "no section" not in body


def test_bad_section_is_rejected_no_path_traversal():
    for evil in ("../whitehat.py", "../../etc/passwd", "/etc/passwd", "nope"):
        out = whitehat.manual(evil)
...
def test_core_fits_a_lowered_output_cap():
    ...
    assert len(whitehat.manual()) < 12000
