...
### 1.3 Captured-traffic workflow (proxy_brain tools)

If HTTP Traffic Capture is enabled, source and drive this survey from the recorded history instead of rebuilding requests by hand. proxy_brain tools only see traffic that went through the capture proxy.

- `whitehat.sitemap()` lists the distinct endpoints already observed; `whitehat.search({"q":"graphql"})` and `whitehat.grep("__schema")` find the API-doc and schema surfaces (`/swagger.json`, `/openapi.json`, `/api-docs`, `/graphql`) that section 1.1 fuzzes for.
- 401/403 bypass (Phase 4.1) runs off one captured forbidden request: `whitehat.replay(<403_txn_id>, {"headers":{"X-Original-URL":"/api/admin/users"}})`, then `{"headers":{"X-Forwarded-For":"127.0.0.1"}}`, then method override `{"headers":{"X-HTTP-Method-Override":"GET"}}` or `{"method":"GET"}`. `whitehat.diff(<403_txn>, <replay_txn>)` shows the 403 -> 200 flip cleanly.
- BOLA/IDOR sequential enumeration (Phase 4.2) and version enumeration (Phase 4.4): `whitehat.fuzz(<captured_txn>, "id", ["1","2","100"])` walks one query parameter (50 payloads max, origin host only) returning per-value status+length. For path-segment IDs (`/api/users/1`) or `/api/vN/` version walks, iterate whitehat.replay with `{"path":...}` instead, since whitehat.fuzz only iterates a query param.
- `whitehat.params()` flags injectable params (sequential-id / uuid / jwt / base64) to prioritise; `whitehat.query({...})` aggregates the observed status matrix; `whitehat.to_curl(id)` renders any hit for the report.

Caveat: whitehat.replay pins host/scheme/port to the origin transaction and cannot retarget a sibling host (use execute_curl for cross-host probes). All proxy_brain tools require HTTP Traffic Capture enabled and only see proxied requests.
