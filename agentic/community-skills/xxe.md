...
When HTTP Traffic Capture is enabled, whitehat.sitemap and whitehat.query surface XML-speaking endpoints already observed (Content-Type xml, `.asmx`/`.svc`, SOAP paths) without re-crawling. whitehat.replay takes one captured XML or JSON transaction and swaps its body for a DOCTYPE/entity payload (set headers to switch Content-Type to application/xml), then whitehat.grep reads a disclosed file (for example /etc/passwd) out of the response, or whitehat.diff compares it against a benign baseline replay. Where the proxy stops: blind OOB exfiltration (external DTD plus interactsh) and XSLT-to-RCE stay in the execute_curl / kali_shell path; whitehat.replay is pinned to the origin host.
