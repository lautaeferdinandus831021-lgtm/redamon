...
If HTTP Traffic Capture is enabled, source and drive this from the recorded history. `whitehat.sitemap` lists the endpoints already observed and `whitehat.search {q:"openapi.json"}` (or `{bodyq:"securitySchemes"}`) finds a captured `/openapi.json` you can `whitehat.get(id,"response")` for the path/security map without re-fetching. Drive the auth and IDOR matrix off real captured requests with `whitehat.replay`: `dropHeaders:["Authorization"]` to test routes missing the security dep, `param:{"id":"FOREIGN"}` (or edit `path`) to swap object IDs, `headers:{"x-tenant-id":"OTHER_ORG"}` to inject a tenant, `headers:{"Content-Type":"application/x-www-form-urlencoded"}` with a re-encoded `body` for the parser differential, and `headers:{"x-forwarded-for":"127.0.0.1"}` to spoof the proxy gate. `whitehat.fuzz(id,"id",[...])` enumerates a sequential/uuid ID over one query param (loop `whitehat.replay` for header or body positions, which `whitehat.fuzz` cannot reach); `whitehat.params` flags hidden or injectable params seen in traffic. `whitehat.diff(owner_id, foreign_id)` gives the owner-vs-foreign delta. Caveats: `whitehat.replay` is pinned to the origin host, and `proxy_brain` only see traffic that went through the capture proxy.
