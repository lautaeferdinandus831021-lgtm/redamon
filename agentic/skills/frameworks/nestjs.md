...
If HTTP Traffic Capture is enabled, source and drive this from the recorded history. `whitehat.sitemap` enumerates the `/api/*` endpoints already seen, and `whitehat.search {q:"api-json"}` (or `{bodyq:"securitySchemes"}`) locates a captured `/api-json` you can `whitehat.get(id,"response")` for the Swagger path/security map. Drive each attack off a real captured request via `whitehat.replay`: `dropHeaders:["Authorization"]` per endpoint to catch a guard that answers 200/403 instead of 401 (guard miss); inject extra DTO fields with a widened `body` (mass assignment under `whitelist` without `forbidNonWhitelisted`); `headers:{"X-HTTP-Method-Override":"DELETE"}` to swap the method before guards run; and `headers:{"Content-Type":"application/x-www-form-urlencoded"}` with a re-encoded body to dodge JSON-only validators. For `CacheInterceptor` poisoning, replay the same path once with the auth `Cookie`/`Authorization` and once with `dropHeaders:["Cookie","Authorization"]`, then `whitehat.diff` the two responses: an identity-free cache serves User A's body to the anon caller. `whitehat.params` surfaces shadowed params and `whitehat.fuzz(id,"id",[...])` pollutes a single query param (loop `whitehat.replay` for `?id=1&id=2`-style duplication or header/body positions). Caveats: `whitehat.replay` is pinned to the origin host, and `proxy_brain` only see traffic that went through the capture proxy.
