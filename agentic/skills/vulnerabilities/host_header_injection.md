...
- `whitehat.replay id mutate:{headers:{"X-Forwarded-Host":"attacker.tld"}}` resends a captured request with a mutated `X-Forwarded-Host` / `X-Original-Host` / `Forwarded` and inspects the reflected host in the body, `Location`, and `Set-Cookie Domain=`. The raw `Host` is pinned to the origin, but `X-Forwarded-Host` (the more-trusted header) is fully mutable.
- `whitehat.diff id_canonical id_poisoned` pairs a clean replay against the poisoned one so the reflected host stands out.
- `whitehat.grep "attacker.tld"` confirms the injected host actually landed in a captured response body.
- `whitehat.search` / `whitehat.sitemap` surface the password-reset, signup, and redirect endpoints worth replaying.
