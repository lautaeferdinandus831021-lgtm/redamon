...
- Harvest tokens: `whitehat.search({"hasAuth":true})` lists every captured request carrying an Authorization header, and `whitehat.grep("eyJ")` / `whitehat.grep("Bearer ")` pull Bearer JWTs out of response bodies and JS bundles. This replaces the `query_graph` token pull.
- `whitehat.get(<txn_id>, "both")` gives the full request/response of a token-bearing transaction, so you can lift the exact header shape and the endpoint that consumes it.
- Replay forged / mutated tokens against the endpoint they came from: forge the variant with `jwt_tool` or the Python recipes below, then `whitehat.replay(<captured_txn>, {"headers":{"Authorization":"Bearer <forged>"}})`. Test alg=none, RS256->HS256, and claim-inflated tokens this way, one per replay.
- Cross-audience acceptance is an auth-swap: `whitehat.replay(<service_B_txn>, {"headers":{"Authorization":"Bearer <token_minted_for_service_A>"}})` proves a token for `aud=billing` is accepted by `aud=admin`. For the gateway-trusts-header gap, `whitehat.replay(id, {"dropHeaders":["Authorization"],"headers":{"X-User-Id":"1"}})`.
- `whitehat.diff(<owner_token_txn>, <forged_token_txn>)` is the two-request PoC the validation section wants: owner-equivalent data returned to the forged token is the finding. `whitehat.to_curl(id)` renders it for the report.

Caveat: whitehat.replay pins host/scheme/port to the origin transaction, so it cannot replay a token against a different host, gRPC port, or WebSocket consumer (use execute_curl / execute_code for those). Browser-side proof (localStorage exfil via XSS) still needs execute_playwright.
