...
If HTTP Traffic Capture is enabled, fingerprint and drive this from the recorded history. `whitehat.search {q:"supabase.co"}` (or `{bodyq:"supabase.co"}`) surfaces the project ref and every PostgREST call the app made; `whitehat.grep "eyJ"` pulls the anon/`service_role` JWTs out of captured bundles, and `whitehat.get(id,"request")` shows the `apikey` header plus the `?id=eq.1`-style filters in the URL. Take a captured `/rest/v1/<table>` read into `whitehat.replay` to swap principals: `headers:{"apikey":"$SERVICE_ROLE","Authorization":"Bearer $SERVICE_ROLE"}` to test a leaked service_role (RLS bypass), or mutate the filter with `query:"select=*&owner_id=eq.OTHER_UID"` / `param:{"org_id":"eq.OTHER_ORG"}` to reach another tenant's rows. `whitehat.fuzz(id,"owner_id",[...])` iterates candidate UIDs over a single filter param (Intruder-style); fuzz the `apikey`/`Authorization` header positions by looping `whitehat.replay` instead, since `whitehat.fuzz` only walks a query param. `whitehat.diff(owner_id, attacker_id)` gives the owner-vs-attacker row delta. Caveats: `whitehat.replay` stays pinned to the origin host, and `proxy_brain` only see traffic that passed through the capture proxy.
