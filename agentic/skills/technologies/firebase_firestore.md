...
If HTTP Traffic Capture is enabled, source and drive this attack from the recorded history instead of re-fetching bundles by hand. `whitehat.grep` the captured response bodies for `firebaseConfig`, `apiKey`, `projectId`, or `databaseURL` to pull the live config straight from JS the target already served; `whitehat.search {bodyq:"firestore.googleapis.com"}` finds the actual Firestore REST calls the app made, and `whitehat.get(id,"both")` shows the exact document path and the ID token in `Authorization`. Feed a captured Firestore GET into `whitehat.replay` to rerun it as the wrong principal: `dropHeaders:["Authorization"]` for the anon probe, or `headers:{"Authorization":"Bearer $TOKEN_A"}` against a path scoped to User B (the auth-context swap the differential below needs). Walk document paths by editing `path` in successive `whitehat.replay` calls, and use `whitehat.params` to spot sequential/uuid document IDs worth enumerating. Then `whitehat.diff(owner_id, foreign_id)` gives the owner-vs-foreign response delta directly. Caveats: `whitehat.replay` is pinned to the origin host, so cross-project/audience-confusion replays to `firestore.googleapis.com` from a captured first-party call only work when the capture already hit Google directly; and `proxy_brain` only see traffic that went through the capture proxy.
