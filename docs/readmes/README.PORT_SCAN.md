# WhiteHat - Port Scanning (Masscan + Naabu)
...
The `naabu_scan.py` module integrates ProjectDiscovery's Naabu scanner into WhiteHat's reconnaissance pipeline. Naabu is optimized for fast, reliable port scanning at scale.
...
**Why default is `True`:** WhiteHat already confirmed hosts exist via DNS resolution. Many firewalls block ICMP ping, causing false negatives.
...
The `masscan_scan.py` module integrates Robert David Graham's Masscan — the fastest Internet port scanner — into WhiteHat's reconnaissance pipeline. Masscan runs as a native binary built from source inside the recon container (not Docker-in-Docker). Both Masscan and Naabu are enabled by default and run in parallel; results are merged and deduplicated automatically.
...
*Documentation updated for WhiteHat v3.1 - Port Scanning Module (Masscan + Naabu)*
