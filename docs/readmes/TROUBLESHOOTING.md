...
WhiteHat is fully Dockerized and runs on **any OS** that supports Docker and Docker Compose v2+. Below are common OS-specific issues and their fixes.
...
| Volume mount fails | Windows path format incompatible with Linux containers | Run from inside WSL2 filesystem (`~/whitehat`), **not** from `/mnt/c/` |
...
| Containers OOM-killed during install | Docker Desktop default Memory below WhiteHat minimum | Docker Desktop → Settings → Resources → Memory: set to 4 GB (8 GB with `--gvm`), then `./whitehat.sh install` |
| `Cannot connect to the Docker daemon` | Docker Desktop not started | `open /Applications/Docker.app` and wait until the whale icon stops animating before running `./whitehat.sh` |
...
| SYN scans (naabu, masscan) miss hosts on the local LAN | `network_mode: host` joins Docker Desktop's LinuxKit VM, not the Mac's network | Expected on Docker Desktop. Internet targets work normally; for LAN scans, run WhiteHat on a Linux host |
| Login fails after install with no error in logs | Non-interactive shell (CI, agents, some multiplexers) injected a `\u0001` SOH byte into the admin email/password prompt | Run `./whitehat.sh create-admin` to (re)create the admin cleanly (`reset-password` only works once an admin already exists) |
| No admin prompt appeared at install / can't log in | Webapp was slow to come up on first boot (common with `--gvm` or a small VM), so the automatic admin prompt was skipped | Run `./whitehat.sh create-admin` once the stack is up (`./whitehat.sh status`); it waits for the webapp, then prompts. Safe to re-run |
...
| HTTP 502 from nginx, `recv() failed (104: Connection reset by peer)` in its error log | The webapp container was OOM-killed mid-response. A reset with no timeout means the process died, rather than the request being slow | Check `./whitehat.sh status` for OOM-killed containers and restart counts. Memory limits are sized from host RAM automatically; see [README.MEMORY_GOVERNOR.md](README.MEMORY_GOVERNOR.md) |
| Containers OOM-killed generally | Host genuinely too small, or limits pinned by hand in `.env` | `./whitehat.sh status` shows the computed allocation and warns when a running container has drifted from it |

Useful checks:

```bash
./whitehat.sh status                                    # OOM kills, restarts, allocation, disk
docker logs whitehat-neo4j --tail 50 | grep -i abort    # Bolt connection aborts
docker inspect whitehat-webapp --format '{{.RestartCount}} {{.State.OOMKilled}}'
```

---

## `./whitehat.sh update` refuses to pull

WhiteHat's own scans used to dirty the git checkout. `recon/` is bind mounted
...
| `Permission denied` writing `.torch-variant`, `.git/index`, or similar | An earlier `sudo ./whitehat.sh …` or `sudo git …` left root-owned files in a user-owned checkout | `sudo chown -R "$(id -un):$(id -gn)" .`, then re-run **without** sudo. `update` now checks this up front rather than failing halfway |
...
inside `whitehat.sh`, which is the file you cannot pull, so a checkout that is
...
files are **all** WhiteHat runtime files is reset automatically and reported,
...
file in the diverging commits must sit under a WhiteHat runtime data directory,
and no path may contain a traversal. To disable it entirely and always recover
by hand, set `WHITEHAT_NO_AUTO_RESET=1`.
